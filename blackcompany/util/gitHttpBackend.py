"""WSGI interface for git-http-backend.

Taken from <https://github.com/walkerh/wsgi-git-http-backend/>:

<https://raw.githubusercontent.com/walkerh/wsgi-git-http-backend/master/gitHttpBackend.py>
"""
...
def wsgi_to_git_http_backend(wsgi_environ,
                             git_project_root,
                             user=None): # pragma: no cover -- TODO external dependency
...
def run_git_http_backend(cgi_environ, input_stream, error_stream): # pragma: no cover -- TODO external dependency
...
def build_cgi_environ(wsgi_environ, git_project_root, user=None): # pragma: no cover -- TODO external dependency
...
def parse_cgi_header(cgi_header): # pragma: no cover -- TODO external dependency
...
def _communicate_with_git(proc, input_stream, input_length): # pragma: no cover -- TODO external dependency
...
def _input_data_pump(proc, input_stream, input_length): # pragma: no cover -- TODO external dependency
...
def _find_header_end_in_2_chunks(chunk0, chunk1): # pragma: no cover -- TODO external dependency
...
def _search_str_for_header_end(data_str): # pragma: no cover -- TODO external dependency
    """Return index of header end or -1."""
    return data_str.find(HEADER_END)


def _separate_header(chunks, header_end_on_boundary, index_within_chunk): # pragma: no cover -- TODO external dependency
...
def _response_body_generator(remainder, proc): # pragma: no cover -- TODO external dependency
